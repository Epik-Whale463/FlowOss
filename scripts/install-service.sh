...
if [ ! -x "$REPO_DIR/target/release/flowoss" ] || [ ! -x "$REPO_DIR/target/release/flowoss-desktop" ]; then
    echo "Build first: cargo build --release" >&2
    exit 1
fi

mkdir -p "$BIN_DIR" "$UNIT_DIR"
# The binaries look for the sherpa-onnx shared libs next to themselves ($ORIGIN).
cp "$REPO_DIR/target/release/flowoss" "$REPO_DIR/target/release/flowoss-desktop" "$BIN_DIR/"
